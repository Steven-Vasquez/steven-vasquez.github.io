...
export function LLMChatbotPage() {
    ...
    const overviewImages = [
        ...
        "/images/Project_Images/LLMChatbot/overview/architecture_diagram.png",
        "/images/Project_Images/LLMChatbot/overview/debug_panel.png",
    ];
    const overviewCaptions = [
        "The chatbot web interface showing the chat panel, session list, and mode selector",
        "High-level architecture diagram of the full system pipeline",
        "The debug output panel surfacing live agent reasoning, tool calls, and LLM thinking steps",
    ];

    // Memory / context pipeline images
    const memoryImages = [
        "/images/Project_Images/LLMChatbot/memory/topic_summary_flow.png",
        "/images/Project_Images/LLMChatbot/memory/chromadb_embeddings.png",
        "/images/Project_Images/LLMChatbot/memory/memory_recall_example.png",
    ];
    const memoryCaptions = [
        "Incremental topic summary updates triggered by the LLM during an active conversation",
        "ChromaDB storing finalized topic embeddings for long-term semantic retrieval",
        "Example of the chatbot successfully recalling context from a previous topic within the same session",
    ];

    // Vanna / Text-to-SQL images
    const vannaImages = [
        "/images/Project_Images/LLMChatbot/vanna/semantic_schema_selection.png",
        "/images/Project_Images/LLMChatbot/vanna/sql_query_output.png",
        "/images/Project_Images/LLMChatbot/vanna/vanna_agent_loop.png",
    ];
    const vannaCaptions = [
        "SemanticSchemaEnhancer selecting the top 5 relevant tables via cosine similarity before the LLM ever sees the prompt",
        "A natural language question translated to a validated SQL query and returned as a structured DataFrame",
        "The Vanna agent loop running follow-up queries when initial results are incomplete (supports up to 25 iterations)",
    ];

    // Web search / RAG images
    const webSearchImages = [
        "/images/Project_Images/LLMChatbot/websearch/search_results_fetched.png",
        "/images/Project_Images/LLMChatbot/websearch/chunk_ranking.png",
        "/images/Project_Images/LLMChatbot/websearch/rag_response_example.png",
    ];
    const webSearchCaptions = [
        "DuckDuckGo search results fetched, cleaned, and parsed by BeautifulSoup into WebDocument objects",
        "Chunks ranked by cosine similarity against the query embedding — top 6 selected with a 2-chunk-per-source cap",
        "Final LLM-synthesized answer citing retrieved source passages, generated by the locally hosted gemma3:12b model",
    ];
    ...
    return (
        <>
            <div className="project-page-container">
                ...
                <h1>How It Works</h1>
                <p>The chatbot operates as a modular pipeline. Each message a user sends passes through a sequence of stages before a response is generated:</p>
                <ol>
                    <li><strong>Message Storage and Embedding</strong> — The user's message is saved to MSSQL for persistence, then embedded and stored in ChromaDB for semantic retrieval.</li>
                    <li><strong>Topic Drift Detection</strong> — The system checks whether the conversation has shifted to a new subject. If it has, the current topic's summary is finalized and written to ChromaDB, and a new incremental summary begins.</li>
                    <li><strong>Prompt Construction</strong> — Relevant conversation history and context summaries are retrieved and assembled into the initial message list for the LLM.</li>
                    <li><strong>LLM Agent Loop</strong> — The LLM processes the current context and decides whether to answer directly or invoke a tool. Tool results are appended to the message list and the loop continues until a final response is ready.</li>
                    <li><strong>Response Delivery</strong> — The final reply is saved back to MSSQL and returned to the user. The memory pipeline updates accordingly, keeping context coherent for the next turn.</li>
                </ol>
                ...
                <p>The <strong>topic drift detection</strong> step runs on every message. If the LLM determines the subject has changed, it triggers the finalization routine before processing the new topic, so no context is lost at the boundary.</p>
                <ImageCarousel carouselImages={memoryImages} captions={memoryCaptions} />
                ...
                <ImageCarousel carouselImages={vannaImages} captions={vannaCaptions} />
                ...
                <ImageCarousel carouselImages={webSearchImages} captions={webSearchCaptions} />
