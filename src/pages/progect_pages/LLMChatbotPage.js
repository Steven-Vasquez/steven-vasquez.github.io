...
export function LLMChatbotPage() {
    ...
    const introDetails = {
        ...
        columnRowsInfo: [
            ...
            ["• Python", "• Flask + FastAPI", "• HTML / JavaScript / CSS", "• ChromaDB", "• MSSQL", "• Ollama", "• Vanna", "• SQLAlchemy", "• DuckDuckGo Search", "• BeautifulSoup", "• nomic-embed-text", "• Qwen3:30b", "• gemma3:12b", "• llama3:7b", "• mistral-nemo:12b"],
        ...
        linkColLinks: ["https://github.com/Steven-Vasquez/LLM-Chatbot"],
    ...
    const overviewImages = [
        ...
        "/images/Project_Images/LLMChatbot/overview/debug_panel_1.png",
        "/images/Project_Images/LLMChatbot/overview/debug_panel_2.png",
        "/images/Project_Images/LLMChatbot/overview/memory_recall_example.png",
    ];
    const overviewCaptions = [
        "The chatbot web interface: session list (left), chat panel (center), live debug output (right). Showing Web Search tool in action.",
    ...
    const vannaImages = [
        "/images/Project_Images/LLMChatbot/vanna/vanna_example.png",
        "/images/Project_Images/LLMChatbot/vanna/vanna_example_logs.png",
    ];

    const vannaCaptions = [
        "The Vanna Text-to-SQL tool's in action.",
        "The Vanna Text-to-SQL tool backend logs. Only the most relevant tables and columns are injected into the prompt based on cosine similarity to the user's natural language query.",
    ];
    ...
    return (
        <>
            <div className="project-page-container">
                ...
                <h1>Key Features</h1>
                <ul>
                    <li><strong>Real-time messaging</strong> — Responses are streamed back to the user as they are generated, with a live WebSocket debug panel exposing the agent's internal reasoning and tool calls.</li>
                    <li><strong>Isolated chat instances</strong> — Each conversation runs independently, allowing multiple simultaneous sessions without any shared state or context bleed between them.</li>
                    <li><strong>Dynamic long-term memory</strong> — A custom memory pipeline maintains incremental topic summaries during active conversations and finalizes them into a vector store when topics change, enabling coherent recall across sessions.</li>
                    <li><strong>Topic drift detection</strong> — The system automatically detects when a conversation has shifted to a new subject, finalizing and storing the previous topic's context before processing the new one.</li>
                    <li><strong>LLM agent loop</strong> — An autonomous multi-step reasoning loop allows the model to invoke tools, evaluate results, and issue follow-up queries before committing to a final response.</li>
                    <li><strong>Text-to-SQL (Vanna)</strong> — Natural language questions are translated into validated SQL queries against a live MSSQL database, with semantic schema selection ensuring only relevant tables are injected into the prompt.</li>
                    <li><strong>Web Search RAG pipeline</strong> — Live DuckDuckGo search results are fetched, chunked, ranked by semantic similarity, and synthesized into a cited answer using a locally hosted LLM.</li>
                    <li><strong>Fully local model stack</strong> — All LLM inference and embedding runs on locally hosted models via Ollama, with no external API calls or data leaving the machine.</li>
                </ul>
                ...
                <p>The design priority was extensibility. Rather than hardcoding any specific behavior, every major capability is modular. This includes the memory pipeline, the SQL tool, the web search tool, and the LLM tools. New agent tools can be dropped in as new types of customer inquiries emerge, without touching the core conversation engine.</p>
                ...
                <h1>How It Works</h1>
                <p>The chatbot operates as a modular pipeline. Each message a user sends passes through a sequence of stages before a response is generated. Green nodes are data storage operations, yellow are LLM agent loop operations, and red are other chatbot functions:</p>
                
                <div className="image-container">
                    <img src="/images/Project_Images/LLMChatbot/message_flow_diagram.svg" alt="End-to-end message flow diagram" className="project-images-large" />
                </div>

                <h1>Memory and Context Management</h1>
                <p>This is the part of the system I'm most proud of. Many chatbots either dump their entire history into every prompt (expensive and eventually impossible) or forget everything after a fixed window. This platform does neither.</p>
                <p>Instead, <strong>topic summaries</strong> are maintained in two states: an <em>incremental</em> summary that is continuously updated by the LLM as a conversation topic progresses, and a <em>finalized</em> summary that is written to ChromaDB the moment the system detects the conversation has moved on. When a user revisits a past topic, the relevant finalized summary is retrieved via semantic search and injected into the prompt, giving the illusion of genuine long-term memory without ever bloating the context window.</p>
                ...
                <div className="image-container">
                    <img src="/images/Project_Images/LLMChatbot/prompt_assembly_diagram.svg" alt="Prompt assembly pipeline diagram" className="project-images-large" />
                </div>
                ...
                <ImageCarousel carouselImages={vannaImages} captions={vannaCaptions}></ImageCarousel>
                <h2>How It Works</h2>
                ...
                <p>The <strong><span style={inlineCode}>SemanticSchemaEnhancer</span></strong> then runs before the LLM ever sees the prompt. It embeds the user's query using <span style={inlineCode}>nomic-embed-text</span> and computes cosine similarity against precomputed embeddings for every table and column in the schema. Each table is scored using a composite formula (using table-level similarity, max column match, and a weighted top-K column average) and the <strong>top five tables</strong> are selected. Their column descriptions and a hardcoded relationship map are injected into the system prompt. Schema embeddings are generated once at setup time from human-written descriptions in <span style={inlineCode}>column_semantics.py</span>, keeping query-time overhead minimal.</p>
                ...
                <div className="image-container">
                    <img src="/images/Project_Images/LLMChatbot/vanna_text2sql_diagram.svg" alt="Vanna Text-to-SQL tool example" className="project-images-large" />
                </div>
                ...
                <div className="image-container">
                    <img src="/images/Project_Images/LLMChatbot/web_search/web_search_example.png" alt="Web Search tool example" className="project-images" />
                </div>
                ...
                <div className="image-container">
                    <img src="/images/Project_Images/LLMChatbot/web_search_diagram.svg" alt="Web Search RAG pipeline diagram" className="project-images-large" />
                </div>
